...
class Moon:
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def get_position(self, epoch: Epoch = Epoch(c.J2000), 
                     fidelity: str|CelestialFidelity = CelestialFidelity.LoFi) -> np.ndarray:
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def get_position_array(self, timeline: Timeline = TIMELINE, 
                           fidelity: CelestialFidelity = CelestialFidelity.LoFi) -> np.ndarray:
    ...
    def _moon_position_low_fidelity(self, julian_dates: np.ndarray) -> np.ndarray:
        ...
        ecliptic_longitude = np.deg2rad(218.32 + 481267.8813*julian_centuries \
        ...
        ecliptic_latitude = np.deg2rad(5.13*np.sin(np.deg2rad(93.3 + 483202.03*julian_centuries)) \
        ...
        parallax = np.deg2rad(0.9508 + 0.0518*np.cos(np.deg2rad(134.9 + 477198.85*julian_centuries)) \
        ...
        obliquity_ecliptic = np.deg2rad(23.439291 - 0.00130042*julian_centuries \
            - (1.64e-7)*julian_centuries**2 + (5.04e-7)*julian_centuries**3)
        
        radius = c.RADIUS_EARTH/np.sin(parallax)
        moon_position_unit_vector = [
            np.cos(ecliptic_latitude)*np.cos(ecliptic_longitude),
            np.cos(obliquity_ecliptic)*np.cos(ecliptic_latitude)*np.sin(ecliptic_longitude) - np.sin(obliquity_ecliptic)*np.sin(ecliptic_latitude),
            np.sin(obliquity_ecliptic)*np.cos(ecliptic_latitude)*np.sin(ecliptic_longitude) - np.cos(obliquity_ecliptic)*np.sin(ecliptic_latitude),
        ]
