...
class Sun:
    ...
    def _sun_position_low_fidelity(self, julian_dates: np.ndarray) -> np.ndarray:
        ...
        radius = 1.000140612 - 0.016708617*np.cos(np.deg2rad(mean_anomaly)) \
                             - 0.000139589*np.cos(np.deg2rad(2*mean_anomaly))
        sun_position_au = [radius*np.cos(np.deg2rad(ecliptic_longitude)),
                        radius*np.cos(np.deg2rad(obliquity_ecliptic))*np.sin(np.deg2rad(ecliptic_longitude)),
                        radius*np.sin(np.deg2rad(obliquity_ecliptic))*np.sin(np.deg2rad(ecliptic_longitude))]
