    ...
    with open(file_path) as input_file:
        for i in range(0, 5):
            ...
            for line in input_file:
                ...
                if dat == []:
                    break
                sum.append(np.array(dat[1:]).astype(float))
...
class EarthOrientationData:
    ...
    def ut1_to_utc(self, mjd):
        ...
        if self._interpolate:
            ...
            # Element-wise linear interpolation of tumples
...
class IERSData:
    ...
    def get_XYs(self, angles, Ttt):
        ...
        # Calculate X
        x = -0.016617 + 2004.191898*Ttt - 0.4297828*(Ttt**2) - 0.19861834*(Ttt*3) + 7.578e-6*(Ttt**4) + 5.9285e-6*(Ttt**5)
        for i, summation in enumerate(self._table_2a):
            summation = np.array(summation)
            Api = np.matmul(summation[:,2:], angles)
            x += np.sum(summation[:,0]*np.sin(Api) + summation[:,1]*np.cos(Api))*(Ttt**(i))*(1e-6)

        # Calculate Y
        y = -0.006951 - 0.025896*Ttt - 22.4072747*(Ttt**2) + 0.00190059*(Ttt**3) + 0.001112526*(Ttt**4) + 1.358e-7*(Ttt**5)
        for i, summation in enumerate(self._table_2b):
            summation = np.array(summation)
            Api = np.matmul(summation[:,2:], angles)
            y += np.sum(summation[:,0]*np.sin(Api) + summation[:,1]*np.cos(Api))*(Ttt**(i))*(1e-6)

        # Calculate s
        s = -(x*y)/2 + 9.4e-5 + 3.80865e-3*Ttt - 1.2268e-4*(Ttt**2) - 0.07257411*(Ttt**3) + 2.798e-5*(Ttt**4) + 1.562e-5*(Ttt**5)
        for i, summation in enumerate(self._table_2d):
            summation = np.array(summation)
            Api = np.matmul(summation[:,2:], angles)
            s += np.sum(summation[:,0]*np.sin(Api) + summation[:,1]*np.cos(Api))*(Ttt**(i))*(1e-6)
...
def precession_nutation(epoch: Epoch):
    ...
    Args:
        epoch (Epoch): Epoch of the Rotation.

    Returns:
        rot_pn (np.ndarray): 3x3 Precession Nutation Rotation Matrix.
    """
    # Get Get Julian Centuries in TT
    Ttt = epoch.julian_centuries('TT')
    Ttt2, Ttt3, Ttt4 = Ttt**2, Ttt**3, Ttt**4

    # Calculate Earth Nutation Angles
    Mms = (485868.249036 + 1717915923.2178*Ttt + 31.8792*Ttt2 + 0.051635*Ttt3 - 0.00024470*Ttt4)*c.AS2DEG
    Ms = (1287104.79305 + 129596581.0481*Ttt - 0.5532*Ttt2 + 0.000136*Ttt3 - 0.00001149*Ttt4)*c.AS2DEG
    uMm = (335779.526232 + 1739527262.8478*Ttt - 12.7512*Ttt2 - 0.001037*Ttt3 + 0.00000417*Ttt4)*c.AS2DEG
    De = (1072260.70369 + 1602961601.2090*Ttt - 6.3706*Ttt2 + 0.006593*Ttt3 - 0.00003169*Ttt4)*c.AS2DEG
    Omega = (450160.398036 - 6962890.5431*Ttt + 7.4722*Ttt2 + 0.007702*Ttt3 - 0.00005939*Ttt4)*c.AS2DEG
    ...
    angles = np.deg2rad([Mms,Ms,uMm,De,Omega,L_Me,L_Ve,L_Ea,L_Ma,L_Ju,L_Sa,L_Ur,L_Ne,pL]) % 2*np.pi
    x, y, s = IERS_DATA.get_XYs(angles, Ttt)
    a = 0.5 + (x**2 + y**2)/8

    # Compute Rotation Matrix for Precession and Nutation
    rot_pn = np.matmul(np.array([
        [1 - a*x**2, -a*x*y, x],
        [-a*x*y, 1 - a*y**2, y],
...
def earth_rotation(epoch: Epoch):
    ...
    Returns:
        rot_pn (np.ndarray): 3x3 Earth Rotation Matrix.
    """
    # Calculate UT1
    jd_utc = epoch.julian_date('UTC') # should be UT1
    utc_to_ut1, _, _ = EOP_DATA.eop(epoch.modified_julian_date('UTC'))
...
def polar_motion(epoch: Epoch):
    ...
    Returns:
        rot_pn (np.ndarray): 3x3 Polar Motion Rotation Matrix.
    """
    # Get Earth Orientation Data
    _, xp, yp = EOP_DATA.eop(epoch.modified_julian_date('UTC'))
    tt = epoch.julian_centuries('TT')
    sp = -0.000047*tt*c.AS2RAD

    # Precomute trig functions
