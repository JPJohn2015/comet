...
def eci_to_ecef(epoch: npt.ArrayLike, state: npt.ArrayLike):
    ...
    for i in range(0, len(epoch)):
        ...
        if len(state[i,:]) == 6:
            ecef_vel = np.matmul(rot_pn, np.matmul(rot_era, np.matmul(rot_pm, state[i,3:6] + np.cross([0, 0, c.OMEGA_EARTH], np.matmul(rot_pm, state[i,0:3])))))
...
def ecef_to_eci(epoch: npt.ArrayLike, state: npt.ArrayLike):
    ...
    for i in range(0, len(epoch)):
        ...
        if len(state[i,:]) == 6:
            eci_vel = np.matmul(rot_pm.T, np.matmul(rot_era.T, np.matmul(rot_pn.T, state[i,3:6])) - np.cross([0, 0, c.OMEGA_EARTH], np.matmul(rot_era.T, np.matmul(rot_pn.T, state[i,0:3]))))
...
def ecef_to_lla(ecef_state: npt.ArrayLike):
    ...
    lat = np.arctan2((z + c.EP_SQ_EARTH*c.B_EARTH * np.sin(theta)**3), (p - c.E_SQ_EARTH*c.A_EARTH*np.cos(theta)**3))
...
def lla_to_ecef(lla_state: npt.ArrayLike, velocity: npt.ArrayLike = None):
    ...
    if velocity is None:
        ecef_state = np.array([x/1000,y/1000,z/1000, np.zeros(x.shape), np.zeros(y.shape), np.zeros(z.shape)]).T
    else:
        ecef_state = np.array([x/1000,y/1000,z/1000, velocity[:,0], velocity[:,1], velocity[:,2]]).T
...
if __name__ == "__main__":
    eci = np.array([[-1033.4793830, 7901.2952754, 6380.3565958, -3.225636520, -2.872451450, 5.531924446],
                   [-1033.4793830, 7901.2952754, 6380.3565958, -3.225636520, -2.872451450, 5.531924446],
                   [-1033.4793830, 7901.2952754, 6380.3565958, -3.225636520, -2.872451450, 5.531924446],
                   [-1033.4793830, 7901.2952754, 6380.3565958, -3.225636520, -2.872451450, 5.531924446]])
    epoch = np.array([Epoch(2004,4,6,7,51,28.386009),
