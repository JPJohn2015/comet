...
class Propagator:
    ...
    def copy(self):
        ...
        """
        return Propagator(self.epoch, self.state, self.force_model, self.sat_properties,
                          self.integrator, self.abs_tol, self.rel_tol, self.min_step, self.max_step)
    ...
    def _integrate(self, relative_time_deltas: np.ndarray):  
        ...
        prop.set_integrator(self.integrator.value, 
    ...
    def from_dict(dict):
        ...
        return Propagator(epoch, state, force_model, sat_properties, integrator,
                              dict['abs_tol'], dict['rel_tol'], dict['min_step'], dict['max_step'])
...
class SpacePropagator(Propagator):
    ...
    def __init__(self, 
        ...
        super().__init__(epoch, state, force_model, sat_properties, integrator,
    ...
    def _integrate(self, relative_time_deltas: np.ndarray):  
        ...
        for maneuver in self.maneuvers:
            ...
            before_end = maneuver.epoch <= self.epoch + Duration(seconds=np.sum(relative_time_deltas))
        ...
        maneuver_time_deltas = [(maneuver.epoch - self.epoch).total_seconds() for maneuver in maneuvers]
        ...
        for i in range(0, len(maneuver_time_deltas)-1):
            ...
            in_section = np.logical_and(time_deltas >= maneuver_time_deltas[i], time_deltas <= maneuver_time_deltas[i+1])
        ...
        for i in range(0, len(section_time_deltas)):
            # Integrate Section
            section_state = self._section_integrate(current_state, current_epoch, section_time_deltas[i])
            ...
            if i != len(section_time_deltas) - 1: 
                if maneuver_at_start:
                    current_state = State(section_state[-1,:]) + np.append([0,0,0], maneuvers[i+1].dv)
                else:
                    current_state = State(section_state[-1,:]) + np.append([0,0,0], maneuvers[i].dv)
                if extra_last_step:
                    current_epoch = current_epoch + Duration(seconds=np.sum(section_time_deltas[i]))
                else:
                    current_epoch = current_epoch + Duration(seconds=np.sum(section_time_deltas[i][:-1]))
    ...
    def from_dict(dict):
        ...
        prop = SpacePropagator(epoch, state, force_model, sat_properties, integrator,
                              dict['abs_tol'], dict['rel_tol'], dict['min_step'], dict['max_step'])
...
class GroundPropagator(Propagator):
    ...
    def __init__(self, 
        ...
        # Initialize Parent Class
        super().__init__(epoch, ground_state, ForceModel(), SatelliteProperties(0, 0, 0, 0, 0), 
                         integrator, abs_tol, rel_tol, min_step, max_step)
        self.lla = state
    ...
    def get_elements(self) -> Elements:
        ...
        Returns:
            elements (Elements): Current Elements.
        """
        raise NotImplementedError('GroundPropagator(): LLA Coordinates cannot be converted to Elements')
    ...
    def get_satellite_properties(self) -> SatelliteProperties:
        ...
        Returns:
            sp (SatelliteProperties): Current SatelliteProperties.
        """
        return NotImplementedError('GroundPropagator(): LLA Coordinates do not have SatelliteProperties')
    ...
    def from_dict(dict):
        ...
        return GroundPropagator(epoch, state, integrator,
                              dict['abs_tol'], dict['rel_tol'], dict['min_step'], dict['max_step'])
...
class SunPropagator(Propagator):
    ...
    def __init__(self, 
        ...
        # Initialize Parent Class
        super().__init__(epoch, sun_state, ForceModel(), SatelliteProperties(0, 0, 0, 0, 0), 
                         fidelity, 1e-6, 1e-6, 10.0, 60.0)
    ...
    def get_elements(self) -> Elements:
        ...
        Returns:
            elements (Elements): Current Elements.
        """
        raise NotImplementedError('SunPropagator(): Sun Coordinates cannot be converted to Elements')
    ...
    def get_satellite_properties(self) -> SatelliteProperties:
        ...
        Returns:
            sp (SatelliteProperties): Current SatelliteProperties.
        """
        return NotImplementedError('SunPropagator(): Sun Coordinates does not have SatelliteProperties')
...
class MoonPropagator(Propagator):
    ...
    def __init__(self, 
        ...
        # Initialize Parent Class
        super().__init__(epoch, moon_state, ForceModel(), SatelliteProperties(0, 0, 0, 0, 0), 
                         fidelity, 1e-6, 1e-6, 10.0, 60.0)
    ...
    def get_elements(self) -> Elements:
        ...
        Returns:
            elements (Elements): Current Elements.
        """
        raise NotImplementedError('MoonPropagator(): Moon Coordinates cannot be converted to Elements')
    ...
    def get_satellite_properties(self) -> SatelliteProperties:
        ...
        Returns:
            sp (SatelliteProperties): Current SatelliteProperties.
        """
        return NotImplementedError('MoonPropagator(): Moon Coordinates does not have SatelliteProperties')
...
class TLEPropagator(Propagator):
    ...
    def __init__(self, 
        ...
        # Initialize Parent Class
        self._tle = tle
        super().__init__(self._tle.epoch(), self._tle.to_state(), force_model, sat_properties, 
                         integrator, abs_tol, rel_tol, min_step, max_step)
    ...
    def from_dict(dict):
        ...
        return TLEPropagator(tle, force_model, sat_properties, integrator,
                              dict['abs_tol'], dict['rel_tol'], dict['min_step'], dict['max_step'])
