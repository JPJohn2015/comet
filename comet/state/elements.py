...
class Elements:
    ...
    def __init__(self, *args):
        ...
        if len(args) == 6:
            ...
            if not np.all([isinstance(arg, int|float|np.int64|np.float64) for arg in args]):
                raise TypeError('Elements(): values must be an integer or float when providing 6 inputs')
    ...
    def flight_path_angle(self) -> float:
        ...
        """
        return np.arctan((self._raw[1]*np.sin(self._raw[5]))/(1 + self._raw[1]*np.cos(self._raw[5])))
