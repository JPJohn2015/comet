...
def cartesian_to_elements(state: npt.ArrayLike, mu: float = c.MU_EARTH):
    ...
    e = (1/mu)*(((norm(vel, axis=-1)**2 - mu/norm(pos, axis=-1))*pos.T).T - (np.sum(pos*vel, axis=-1)*vel.T).T)
    ...
    v = np.where(e_hat == 0.0, 0.0, np.arccos(np.sum(e*pos, axis=-1)/(norm(e, axis=-1)*norm(pos, axis=-1))))
...
def elements_to_cartesian(elements: npt.ArrayLike, mu: float = c.MU_EARTH):
    ...
    r_pqw = np.array([(p*np.cos(elements[...,5]))/(1 + elements[...,1]*np.cos(elements[...,5])), 
                      (p*np.sin(elements[...,5]))/(1 + elements[...,1]*np.cos(elements[...,5])), 
                      np.zeros((len(p)))]).T
    v_pqw = np.array([-np.sqrt(mu/p)*np.sin(elements[...,5]), 
    ...
    PQW_ECI = np.array([[cRAAN*cW - sRAAN*sW*cI, -cRAAN*sW - sRAAN*cW*cI, sRAAN*sI],
